...
SHERLOCK_MARK = "Generated by [Sherlock]"
...
def find_cold_cases(mcp: DataHubMCP, limit: int = 20, query: str = "*") -> list[ColdCase]:
    ...
    for ent in entities:
        ...
        if desc and SHERLOCK_MARK in str(desc):
            # case already solved by a previous patrol — knowledge is in the graph
            continue
        if not desc or not str(desc).strip():
            missing.append("description")

        # column docs live in BOTH native schema and the editable layer;
        # list_schema_fields merges them (description + editedDescription)
        try:
            sf = mcp.call("list_schema_fields", urn=urn)
            fields = sf.get("fields", []) if isinstance(sf, dict) else []
        except Exception:
            fields = _get(ent, "schema", "fields", default=[]) or []
        if fields:
            undocumented = [
                f for f in fields
                if not ((f.get("description") or f.get("editedDescription") or "").strip())
            ]
